...
public interface Node<V> {
  ...
  /**
   * Whether a reference to this node should be represented as a hash of the rlp,
   * or the node rlp
   * itself should be inlined (the rlp stored directly in the parent node). If
   * true, the node is
   * referenced by hash. If false, the node is referenced by its rlp-encoded
   * value.
   *
  ...
  /**
   * Marking a node as need heal means that one of its children is not yet present
   * in the storage
   */
  void markHealNeeded();
}
