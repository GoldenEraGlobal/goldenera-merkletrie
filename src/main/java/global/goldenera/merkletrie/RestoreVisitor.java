...
import global.goldenera.merkletrie.patricia.DefaultNodeFactory;
import global.goldenera.merkletrie.patricia.ExtensionNode;
import global.goldenera.merkletrie.patricia.LeafNode;
...
public class RestoreVisitor<V> implements PathNodeVisitor<V> {
  ...
  public Node<V> visit(final ExtensionNode<V> extensionNode, final Bytes path) {
    ...
    final Node<V> updatedExtension = extensionNode.replacePath(extensionPath.slice(commonPathLength + 1));
    final Node<V> leaf = nodeFactory.createLeaf(leafPath, value);
    final Node<V> branch = nodeFactory.createBranch(leafIndex, leaf, extensionIndex, updatedExtension);
  ...
  private BranchNode<V> persistNode(final BranchNode<V> parent, final byte index) {
    ...
    if (!(child instanceof StoredNode)) {
      ...
      final PersistedNode<V> persistedNode = new PersistedNode<>(null, child.getHash(), child.getEncodedBytesRef());
