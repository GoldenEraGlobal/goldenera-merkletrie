...
public abstract class CompactEncoding {
  ...
  /**
   * Converts a byte sequence into a path by splitting each byte into two nibbles.
   * The resulting
  ...
  /**
   * Converts a path into a byte sequence by combining each pair of nibbles into a
   * byte. The path
   ...
   * @throws IllegalArgumentException if the path is empty or not a leaf path, or
   *                                  if it contains
  ...
  /**
   * Encodes a path into a compact form. The encoding includes a metadata byte
   * that indicates
   * whether the path is a leaf path and whether its length is odd or even.
   *
   * @param path the path to encode
   * @return the encoded path
   * @throws IllegalArgumentException if the path contains elements larger than a
   *                                  nibble
   */
  ...
  /**
   * Decodes a path from its compact form. The decoding process takes into account
   * the metadata byte
   * that indicates whether the path is a leaf path and whether its length is odd
   * or even.
   *
   * @param encoded the encoded path to decode
   * @return the decoded path
   * @throws IllegalArgumentException if the encoded path is empty or its metadata
   *                                  byte is invalid
   */
