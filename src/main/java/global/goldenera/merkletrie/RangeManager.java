...
import global.goldenera.merkletrie.exceptions.MerkleTrieException;
...
/**
 * This class helps to generate ranges according to several parameters (the
...
public class RangeManager {
  ...
  public static final Hash MAX_RANGE = Hash
  ...
  /**
   * Helps to create a new range according to the last data obtained. This happens
   * when a peer
  ...
  public static Optional<Bytes32> findNewBeginElementInRange(
    ...
    if (receivedKeys.isEmpty() || receivedKeys.lastKey().compareTo(endKeyHash) >= 0) {
      ...
      final StoredMerklePatriciaTrie<Bytes, Bytes> storageTrie = new StoredMerklePatriciaTrie<>(
  ...
  /**
   * Checks if a given location is within a specified range. This method
   * determines whether a given
   * location (represented as {@link Bytes}) falls within the range defined by a
   * start key path and
   * an end key path.
   *
   * @param location     The location to check, represented as {@link Bytes}.
   * @param startKeyPath The start of the range as path, represented as
   *                     {@link Bytes}.
   * @param endKeyPath   The end of the range as path, represented as
   *                     {@link Bytes}.
   * @return {@code true} if the location is within the range (inclusive);
  ...
  /**
   * Transforms a list of bytes into a path. This method processes a sequence of
   * bytes, expanding
   * each byte into two separate bytes to form a new path. The resulting path will
   * have twice the
