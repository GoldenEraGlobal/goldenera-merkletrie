...
public interface MerkleStorage {
  ...
  /**
   * Returns an {@code Optional} of the content mapped to the hash if it exists;
   * otherwise empty.
   *
   * @param location The location in the trie.
   * @param hash     The hash for the content.
   * @return an {@code Optional} of the content mapped to the hash if it exists;
   *         otherwise empty
   */
  Optional<Bytes> get(Bytes location, Bytes32 hash);

  /**
   * Updates the content mapped to the specified hash, creating the mapping if one
   * does not already
   * exist.
   *
   * <p>
   * Note: if the storage implementation already contains content for the given
   * hash, it will
